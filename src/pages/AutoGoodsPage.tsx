...
const AUTO_CATALOG: AutoItem[] = [
  ...
  { id: "acc-3",   category: "Аксессуары",            name: "Автомобильный видеорегистратор", brand: "70mai",        price: 4900,  unit: "шт",    inStock: true,  desc: "4K видеорегистратор с GPS, ночное видение, Wi-Fi",                            icon: "Camera" },
  { id: "acc-4",   category: "Аксессуары",            name: "Автохимия очиститель стёкол", brand: "Liqui Moly",     price: 320,   unit: "500 мл",inStock: true,  desc: "Концентрат для омывателя, -30°С",                                             icon: "Sparkles" },
];
...
export default function AutoGoodsPage({ onNavigate }: AutoGoodsPageProps) {
  ...
  const handleOrder = async () => {
    ...
    try {
      // Списываем с кошелька через wallet API (admin_adjust type spend)
      ...
      // admin_adjust требует роль admin — используем кошелёк через spend транзакцию напрямую
      // Пока показываем заявку — реальная оплата через менеджера
      setOrderMsg(`✓ Заявка на «${selected.name}» оформлена! Наш менеджер свяжется с вами для подтверждения и списания ${selected.price.toLocaleString("ru-RU")} ₽ с кошелька.`);
        ...
        <div className="container mx-auto">
          ...
          <div className="flex items-end justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-muted-foreground text-sm">Запчасти, масла, аксессуары с оплатой с кошелька или под заказ</p>
            </div>
            {user && (
      ...
      <div className="container mx-auto section-py">
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(item => (
              <div key={item.id} onClick={() => { setSelected(item); setOrderMsg(""); }}
                ...
                <div className="flex items-center justify-between pt-3 border-t border-border">
                  ...
                  <button className="btn-ghost text-xs py-1.5 px-3">
                    <Icon name="ShoppingCart" size={13} />{item.inStock ? "Заказать" : "Под заказ"}
                  </button>
                </div>
              </div>
            ))}
        ...
        <div className="mt-12 grid sm:grid-cols-3 gap-4">
          {[
            ...
            { icon: "Wallet",  title: "Оплата с кошелька",     desc: "Спишем с баланса при подтверждении заказа менеджером" },
              ...
              <div className="flex items-center justify-between py-3 border-t border-border">
                ...
                {balance !== null && (
                  <div className="text-right">
                    ...
                    <div className={`font-display font-bold text-lg ${balance >= selected.price ? "text-green-400" : "text-destructive"}`}>
              ...
              {!user ? (
                ...
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground">
                    Не хватает <span className="text-destructive font-bold">{(selected.price - balance).toLocaleString("ru-RU")} ₽</span>
                  </p>
                  <div className="flex gap-3">
                    <button onClick={() => { setSelected(null); onNavigate("account"); }} className="btn-ghost flex-1 justify-center text-xs py-2.5">
                      <Icon name="Wallet" size={13} />Пополнить кошелёк
                    </button>
                    <button onClick={handleOrder} disabled={ordering || orderMsg.startsWith("✓")}
                      className="btn-ghost flex-1 justify-center text-xs py-2.5 border-amber-500/40 text-amber-400 hover:bg-amber-500/10 disabled:opacity-60">
                      {ordering ? <Icon name="Loader2" size={13} className="animate-spin"/> : <Icon name="ClipboardList" size={13}/>}
                      Оставить заявку
                    </button>
                  </div>
                </div>
                ...
                <button onClick={() => setSelected(null)} className="btn-ghost w-full justify-center">
                  <Icon name="Check" size={15} />Заявка принята
                </button>
              ) : (
                <button onClick={handleOrder} disabled={ordering}
                  className="btn-green w-full justify-center disabled:opacity-60">
                  {ordering
                    ? <><Icon name="Loader2" size={15} className="animate-spin" />Оформляем...</>
                    : <><Icon name="ShoppingCart" size={15} />Заказать — {selected.price.toLocaleString("ru-RU")} ₽</>
                  }
                </button>
              )}
            </div>
