...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        user_id, user_name, user_role = user[0], user[1], user[2]

        # GET contacts — список диалогов
        if method == "GET" and action == "contacts":
            cur.execute(f"""
                SELECT DISTINCT
                  ...
                  (SELECT body FROM {SCHEMA}.messages WHERE (from_user_id = %s AND to_user_id = partner_id) OR (from_user_id = partner_id AND to_user_id = %s) ORDER BY created_at DESC LIMIT 1) AS last_msg,
                  (SELECT created_at FROM {SCHEMA}.messages WHERE (from_user_id = %s AND to_user_id = partner_id) OR (from_user_id = partner_id AND to_user_id = %s) ORDER BY created_at DESC LIMIT 1) AS last_at,
                  (SELECT COUNT(*) FROM {SCHEMA}.messages WHERE from_user_id = partner_id AND to_user_id = %s AND is_read = false) AS unread
            ...
            rows = cur.fetchall()
            contacts = [{"id": r[0], "name": r[1], "role": r[2], "last_message": r[3], "last_at": str(r[4]) if r[4] else None, "unread": r[5]} for r in rows]
            return {"statusCode": 200, "headers": cors_headers(), "body": json.dumps({"contacts": contacts})}

        # GET users — список пользователей для нового диалога (только для admin — все, для user — список admin)
        if method == "GET" and action == "users":
            if user_role == "admin":
                cur.execute(f"SELECT id, name, role FROM {SCHEMA}.users WHERE id != %s AND is_active = true ORDER BY name", (user_id,))
            else:
                cur.execute(f"SELECT id, name, role FROM {SCHEMA}.users WHERE role = 'admin' AND is_active = true ORDER BY name")
            rows = cur.fetchall()
            users = [{"id": r[0], "name": r[1], "role": r[2]} for r in rows]
            return {"statusCode": 200, "headers": cors_headers(), "body": json.dumps({"users": users})}

        # GET messages — история переписки с конкретным пользователем
        ...
        if method == "POST" and action == "send":
            ...
            file_type = body.get("file_type", "")  # image/video/file
            ...
            cur.execute(
                f"INSERT INTO {SCHEMA}.messages (from_user_id, to_user_id, body, file_url, file_type, file_name) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id, created_at",
        ...
        # GET unread_count — кол-во непрочитанных
        if method == "GET" and action == "unread":
            cur.execute(f"SELECT COUNT(*) FROM {SCHEMA}.messages WHERE to_user_id = %s AND is_read = false", (user_id,))
